...
float iou(float lbox[4], float rbox[4]) {
  float interBox[] = {
    ...
    // 如果我们包含了含有这些 max 或 min 定义的头文件，那么我们在使用<algorithm>库中提供的std::max()时就会冲突，编译器报错： “C2589 “(”:“::”右边的非法标记 ”
    // 而解决的方法也非常简单，我们只需要在 std::max 上加上括号
    (std::min)(lbox[0] + lbox[2] / 2.f , rbox[0] + rbox[2] / 2.f), //right
    (std::max)(lbox[1] - lbox[3] / 2.f , rbox[1] - rbox[3] / 2.f), //top
    (std::min)(lbox[1] + lbox[3] / 2.f , rbox[1] + rbox[3] / 2.f), //bottom
  };
  // 如果相交部分的上下左右边界不符合逻辑（即相交部分不存在），则返回 0.0f。
  if (interBox[2] > interBox[3] || interBox[0] > interBox[1])
    return 0.0f;
  // 相交部分的宽度是 right 边界减去 left 边界，高度是 bottom 边界减去 top 边界。
