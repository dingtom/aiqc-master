...
struct AffineMatrix
{
  float value[6];  // 声明了一个名为 value 的数组，它可以存储六个 float 类型的元素。
};

// 一个线程处理一个像素点
__global__ void preprocess_kernel(
  // 对每个像素点的三个通道进行归一化，bgr2rgb，NHWC2NCHW
    ...
    int dst_height, int edge)
{
  ...
  float *pdst_c0 = dst + dy * dst_width + dx;    // 指向目标图像中红色通道数据的指针
    ...
    AffineMatrix d2s, int edge)
{
  ...
  // 从d2s中读取变换矩阵
  float m_x1 = d2s.value[0];  // 缩放，旋转，平移
    ...
    float *dst, int dst_width, int dst_height)
{
  ...
  s2d.value[0] = scale;   // 用于 x 方向的缩放。
  s2d.value[1] = 0;       // 表示没有旋转。
  s2d.value[2] = -scale * src_width * 0.5 + dst_width * 0.5;  // 用于 x 方向的平移
  s2d.value[3] = 0;   // 表示没有旋转。
  s2d.value[4] = scale;   // 用于 y 方向的缩放
  s2d.value[5] = -scale * src_height * 0.5 + dst_height * 0.5;   // 用于 y 方向的平移
  ...
  // ptr<float>(0) 指向矩阵第一个元素的指针。这里使用 float 作为模板参数，表示矩阵元素类型是 float。
...
void cuda_preprocess_init(int max_image_size)
{
  ...
  // cudaMalloc 第一个参数是一个指向 img_buffer_device 的指针的指针。
  // img_buffer_device 是一个指针，它将指向 GPU 上分配的内存。
  // 使用 (void **) 是为了告诉 cudaMalloc 函数 img_buffer_device 是一个通用指针（void *）。
}
...
void process_input_gpu(cv::Mat &src, float *input_device_buffer)
{
  ...
  // src.ptr(): 这个表达式返回一个指向 cv::Mat src 的第一个元素的指针。由于没有指定通道或深度，这通常意味着返回指向矩阵第一个元素的指针。
}
